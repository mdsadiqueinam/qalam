...
<template>
  <component
    :is="component"
    class="items-top z-10 flex w-full transform gap-2 overflow-hidden rounded-xl border border-divider bg-main-unselected px-3 py-2 text-[13px] shadow-2xl ring-1 ring-black/5 backdrop-blur-sm transition-all hover:border-divider-hover"
  ...
  >
    <div class="size-5">
      ...
      <ExclamationTriangleIcon
        v-else-if="props.toast.type === 'warning'"
        class="size-5 text-warning"
      />
      <InformationCircleIcon v-else class="size-5 text-primary" />
    </div>
